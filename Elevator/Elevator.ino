...
rf24_pa_dbm_e pa_lvl = RF24_PA_MIN;
...
void setup(void)
{
  ...
  //Serial.print("ROLE: ") + role_friendly_name[role]);
...
void loop(void)
{
  ...
  //
   ...
   if( pa_lvl == RF24_PA_MAX ){
     pa_lvl = RF24_PA_MIN;
   }
   
   pa_lvl = (rf24_pa_dbm_e) ( (int)pa_lvl + 1 );
   Serial.println("Pa lvl now at " + pa_lvl );
  ...
  if (role == role_ping_out)
  {
    ...
    //Serial.println("Now sending %lu...",time);
  ...
  if ( role == role_pong_back )
  {
    ...
    if ( radio.available() )
    {
      ...
      while (!done)
      {
        ...
        // Spew it
        //Serial.println("Got payload %lu...",got_time);
